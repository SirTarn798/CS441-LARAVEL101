<x-layouts.main>
    ...
    <form class="inline-block" action="{{ route('artists.destroy', ['artist' => $artist]) }}" method="POST">
        @csrf
        @method('DELETE')
        <button type="submit" disabled="{{$artist->songs->isEmpty()}}"
            class="inline-block py-2 px-4 border border-gray-700 bg-pink-100 {{$artist->songs->isEmpty() ? 'cursor-pointer' : 'cursor-not-allowed'}}">
            Delete Artist
        </button>
